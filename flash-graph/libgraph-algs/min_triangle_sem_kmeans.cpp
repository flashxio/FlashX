...
namespace {
...
#if KM_TEST
    static clustercore::prune_stats::ptr g_prune_stats;
    ...
    static clustercore::activation_counter::ptr acntr; // How many are active per iteration
    static clustercore::active_counter::ptr ac;
#endif
    static size_t g_io_reqs = 0;

    static bool g_prune_init = false;
    static kprune::dist_matrix::ptr g_cluster_dist;
    static clustercore::prune_clusters::ptr g_clusters; // cluster means/centers

    static unsigned NUM_ROWS;
    static unsigned g_num_changed = 0;
    static struct timeval start, end;
    static clustercore::init_t g_init; // May have to use
    ...
    static clustercore::stage_t g_stage; // What phase of the algo we're in
    static clustercore::dist_t g_dist_type;
    ...
    class kmeans_vertex: public base_kmeans_vertex
    {
        ...
        void run(vertex_program& prog, const page_vertex &vertex) {
            switch (g_stage) {
                case clustercore::stage_t::INIT:
                    run_init(prog, vertex, g_init);
                    break;
                case clustercore::stage_t::ESTEP:
    ...
    class kmeans_vertex_program:
        public base_kmeans_vertex_program<kmeans_vertex, clustercore::clusters> {

        private:
        graph_engine::ptr mat;
        unsigned num_reqs;
#if KM_TEST
        clustercore::prune_stats::ptr pt_ps;
#endif
        ...
        kmeans_vertex_program(graph_engine::ptr mat) {
            ...
            this->num_reqs = 0;
#if KM_TEST
            pt_ps = clustercore::prune_stats::create(NUM_ROWS, K);
#endif
        }
...
#if KM_TEST
        clustercore::prune_stats::ptr get_ps() { return pt_ps; }
#endif
        ...
        virtual void run_on_iteration_end() override {
            if (iter_barrier->ping()) { // Make sure all partitions are complete 1st
                if (!g_converged) {
                    ...
                    clustercore::print(g_num_members_v);
    ...
    void kmeans_vertex::run(vertex_program &prog) {
        ...
        } else if (g_stage != clustercore::stage_t::INIT) { // Always get here when not INIT
        ...
        if (g_stage != clustercore::stage_t::INIT)
            ...
            case clustercore::init_t::PLUSPLUS:
                {
                    ...
                    if (g_kmspp_stage == ADDMEAN) {
                        ...
                        if (get_cluster_id() != INVALID_CLUST_ID &&
                            ...
                            double _dist = clustercore::dist_comp_raw(
    ...
    void kmeans_vertex::run_init(vertex_program& prog,
            const page_vertex &vertex, clustercore::init_t init) {
        switch (g_init) {
            case clustercore::init_t::RANDOM:
                {
                    unsigned new_cluster_id = random() % K;
    ...
    void kmeans_vertex::run_distance(vertex_program& prog, const double* row) {
        ...
        if (g_prune_init) {
            for (unsigned cl = 0; cl < K; cl++) {
                double udist = clustercore::dist_comp_raw(
            ...
            for (unsigned cl = 0; cl < K; cl++) {
                ...
                if (!recalculated) {
                    double udist = clustercore::dist_comp_raw(row, &(g_clusters->get_means()
    ...
    static void clear_clusters() {
        if (g_prune_init) {
            ...
            for (unsigned cl = 0; cl < K; cl++) {
                ...
                clustercore::print<double>(g_clusters[cl]->get_mean());
#endif
            }
        }
    }
    ...
    void update_clusters(graph_engine::ptr mat,
        ...
        for (unsigned thd = 0; thd < kms_clust_progs.size(); thd++) {
            ...
            clustercore::clusters::ptr pt_clusters = kms_prog->get_pt_clusters();
        ...
        for (unsigned cl = 0; cl < K; cl++) {
            ...
            g_clusters->set_prev_dist(clustercore::eucl_dist(&(
...
namespace fg
{
    void compute_min_triangle_sem_kmeans(FG_graph::ptr fg, const unsigned k,
        ...
        BOOST_LOG_TRIVIAL(info) << "Matrix has rows = " << NUM_ROWS << ", cols = " <<
            NUM_COLS;
#if KM_TEST
        g_prune_stats = clustercore::prune_stats::create(NUM_ROWS, K);
        acntr = clustercore::activation_counter::create(g_nthread);
#endif
#if VERBOSE
        ...
        g_dist_type = clustercore::dist_t::EUCL; // TODO: Add to params
        ...
        g_clusters = clustercore::prune_clusters::create(K, NUM_COLS);
        if (centers)
        ...
        if (!centers) {
            g_stage = clustercore::stage_t::INIT;

            if (init == "random") {
                BOOST_LOG_TRIVIAL(info) << "Running init: '"<< init <<"' ...";
                g_init = clustercore::init_t::RANDOM;
            ...
            if (init == "forgy") {
                ...
                g_init = clustercore::init_t::FORGY;
                ...
                g_init = clustercore::init_t::PLUSPLUS;
        ...
        g_stage = clustercore::stage_t::ESTEP;
        if (max_iters > 0) {
            mat->start_all(vertex_initializer::ptr(),
                    vertex_program_creater::ptr(
                        new kmeans_vertex_program_creater(mat)));
