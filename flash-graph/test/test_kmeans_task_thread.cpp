...
static void wake4run(std::vector<prune::kmeans_task_thread::ptr>& threads,
...
static void test_thread_creation(const unsigned NTHREADS, const unsigned nnodes) {
    ...
    for (unsigned i = 0; i < NTHREADS; i++) {
        prune_clusters::ptr cl = prune_clusters::create(2,2);
        threads.push_back(prune::kmeans_task_thread::create
...
void test_numa_populate_data() {
    ...
    std::vector<prune::kmeans_task_thread::ptr>::iterator it = threads.begin();
    // Print it back
