...
template <typename T>
void print_FG_vector(typename FG_vector<T>::ptr fgv) {
  std::cout << "[";
  ...
  for (vsize_t iter = 0; iter < g_MAX_ITERS; iter++) {
    ...
    for (vsize_t vertexID = 0; vertexID < eigs->get_num_rows(); vertexID++) {
      for (vsize_t clusterID = 0; clusterID < clusters->get_num_rows(); clusterID++) {
        ...
        // print_FG_vector<ev_float_t>(clusters->get_row_ref(clusterID));
      ...
      // print_FG_vector<ev_float_t>(distance_matrix->get_row_ref(vertexID));
    ...
    // printf("Previous clusters: "); print_FG_vector<vsize_t>(prev_cluster_assignments);
    // printf("\nNew clusters: "); print_FG_vector<vsize_t>(cluster_assignments); 
    
    if (cluster_assignments->eq_all(prev_cluster_assignments)) {
    ...
    // Set prev to current
    prev_cluster_assignments->shallow_copy(cluster_assignments);  
  }
