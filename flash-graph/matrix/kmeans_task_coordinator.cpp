...
task* kmeans_task_coordinator::steal_task(bool _is_numa, const unsigned node_id) {
    if (_is_numa) {
        // TODO
    } else {

    }
    return new task(NULL, 0, 0);
}
...
void kmeans_task_coordinator::run_init() {
    switch(_init_t) {
        case km::init_type_t::RANDOM:
            random_partition_init();
            break;
        case km::init_type_t::FORGY:
            forgy_init();
            break;
        case km::init_type_t::PLUSPLUS:
            kmeanspp_init();
            break;
        case km::init_type_t::NONE:
            break;
        default:
