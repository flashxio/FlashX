		...
		user_comp_req_queue &reqs)
{
	...
	if (compute->get_ref() == 0 && !compute->has_requests()) {
...
{
	comp_allocator->set_compute(compute);
	compute->fetch_requests(this, comp_allocator, requests);
	comp_allocator->set_compute(NULL);

	if (compute->has_requests()) {
		assert(incomplete_compute == NULL);
		incomplete_compute = compute;
		return false;
	}
...
{
	// If we don't have requests to process or we have an incomplete
	// computation, which means user computation has generated too many
	// requests.
	if (complete_queue.is_empty() || incomplete_compute)
		return 0;
	...
	while (!complete_queue.is_empty()) {
		...
		if (reqp->get_req_type() == io_request::USER_COMPUTE) {
			...
			bool ret = complete_user_compute(compute, requests);
			// If we can't fetch all requests generated by the computation,
			// we should stop performing other computations.
			if (!ret)
				break;
		}
...
void global_cached_io::process_cached_reqs(user_comp_req_queue &requests)
{
	...
	// computation, which means user computation has generated too many
	// requests.
	if (cached_requests.is_empty() || incomplete_compute)
		return;
...
void global_cached_io::process_all_requests()
{
	...
	user_compute *compute = incomplete_compute;
	incomplete_compute = NULL;
	if (compute)
		complete_user_compute(compute, user_comp_requests);

	// We process requests when there are requests served in the page cache
	while ((!cached_requests.is_empty()
				// or requests completed in the underlying IO
				|| !complete_queue.is_empty())
			// If we don't have incomplete computation, it means we still
			// haven't generated too many user compute.
			&& incomplete_compute == NULL) {
		...
		// Now we may have processed some requests. We should try to fetch
		// more requests from the incomplete computation if possible.
		user_compute *compute = incomplete_compute;
		incomplete_compute = NULL;
		if (compute)
			complete_user_compute(compute, user_comp_requests);
