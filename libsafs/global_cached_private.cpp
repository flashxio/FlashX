 ...
 * will be returned to global_cached_io in a sorted order.
 *
	...
	do {
		...
		for (; it != end; ++it) {
			...
			int ret = compute->fetch_requests(get_io(), requests,
					min(max - num, min(MAX_REQS_FETCH, requests.get_num_remaining())));
			num += ret;
			...
			has_reqs |= compute->has_requests();
		}
	} while (has_reqs && !requests.is_full() && num < max);
	assert(num <= max);
	return num;
}
 ...
 */
void global_cached_io::complete_user_compute(user_compute *compute)
{
	...
	if (has_requests || !has_completed)
		comp_io_sched->add_compute(compute);
	...
	user_comp_requests(underlying->get_node_id(), 512)
{
	...
	if (sched == NULL)
		comp_io_sched = new default_comp_io_scheduler(this->get_node_id());
	else
		comp_io_sched = sched;
