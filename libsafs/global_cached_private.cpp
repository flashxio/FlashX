...
public:
	join_compute_allocator(int node_id) {
		...
		allocator = new obj_allocator<join_compute>("join-compute-allocator",
				// We prepare twice the size of memory, in case we need to
				// allocate more objects than we expect.
				// TODO but it's not good enough. It's better to make sure
				// we never allocate more objects than we expect.
...
int global_cached_io::process_completed_requests(user_comp_req_queue &requests)
{
	if (complete_queue.is_empty()
			|| requests.get_num_entries() >= params.get_max_join_compute())
		return 0;
	...
	while (!complete_queue.is_empty()
		...
		if (reqp->get_req_type() == io_request::USER_COMPUTE) {
			...
			reqp->compute(this, comp_allocator, requests);
		}
...
{
	// If we don't have requests to process or we have got enough requests
	// from user compute.
...
void global_cached_io::process_all_requests()
{
	...
	while ((!cached_requests.is_empty()
			...
			// But we have to make sure that we don't have too many requests
			// generated by user compute. Otherwise, we may use too much memory.
			&& user_comp_requests.get_num_entries() < params.get_max_join_compute()) {
