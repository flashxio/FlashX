...
public:
	...
	void run(page_byte_array &array) {
		assert(compute != NULL);
		compute->run(array);
		compute->dec_ref();
...
void global_cached_io::process_incomplete_computes(user_comp_req_queue &requests)
{
	...
	for (int i = 0; i < size; i++) {
		...
		int ret = compute->fetch_requests(this, comp_allocator, requests);
		...
		if (compute->has_completed()) {
			// If we have completed the computation, we can destroy it.
			// If no one else is referencing the user compute, it means
			// the computation is complete now.
			...
			// If the computation didn't issue any new requests, it must have
			// some pending requests. Otherwise, they can't be executed again.
			if (ret == 0)
				assert(compute->get_ref() > 0);
		...
		user_comp_req_queue &requests)
{
	...
	if (compute->has_requests() || !compute->has_completed()) {
		...
		assert(ret > 0);
		// Only the computation defined by the applications can be added
		// to the queue. join_compute will never be added to the queue.
		// An application-defined computation is never added to the queue
		// twice. It's added to the queue only at the first time when it
		// requests data. Later on, it's always executed through join_compute.
		// TODO we have to make sure a computation is never added to the queue
		// twice.
		incomplete_computes.push_back(compute);
		return false;
	}
...
global_cached_io::global_cached_io(thread *t, io_interface *underlying,
	...
	incomplete_computes(underlying->get_node_id(),
			params.get_max_join_compute(), true)
{
