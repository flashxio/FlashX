...
public:
	...
	virtual size_t get_requests(fifo_queue<io_request> &reqs);
};

size_t default_comp_io_scheduler::get_requests(fifo_queue<io_request> &requests)
{
	...
	do {
		...
		for (; it != end; ++it) {
			...
			int ret = compute->fetch_requests(get_io(), requests,
					min(MAX_REQS_FETCH, requests.get_num_remaining()));
			num += ret;
	...
	} while (has_reqs && !requests.is_full());
	return num;
}
