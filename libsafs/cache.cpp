...
void page_byte_array::memcpy(off_t rel_off, char buf[], size_t size) const
{
	...
	if (end - page_begin > PAGE_SIZE) {
		...
		size_t copied = 0;
		::memcpy(buf, get_page(pg_idx) + off_in_pg, part1_size);
		copied += part1_size;
		pg_idx++;
		while (pg_idx < last_pg_idx) {
			::memcpy(buf + copied, get_page(pg_idx), PAGE_SIZE);
			copied += PAGE_SIZE;
			pg_idx++;
		}
		size_t part2_size = size - copied;
		if (part2_size > 0)
			::memcpy(buf + copied, get_page(pg_idx), part2_size);
	}
	else {
		off_t pg_idx = off / PAGE_SIZE;
		off_t off_in_pg = off % PAGE_SIZE;
		::memcpy(buf, get_page(pg_idx) + off_in_pg, size);
	}
}
