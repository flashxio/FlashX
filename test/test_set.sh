...
./test/test_rand_io test/conf/run_remote_virt.txt test

echo "this is the basic test for remote IO for writes to virtual SSDs."
./test/test_rand_io test/conf/run_remote_virt.txt test read_percent=0
...
./test/test_rand_io test/conf/run_remote_virt.txt test entry_size=$((4096 * 32)) RAID_block_size=64K

echo "this is the basic test for remote IO write to virtual SSDs with verification enabled. Large write."
./test/test_rand_io test/conf/run_remote_virt.txt test entry_size=$((4096 * 32)) read_percent=0 RAID_block_size=64K
...
./test/test_rand_io test/conf/run_cache_virt.txt test

echo "the basic test for global cached write on virtual SSDs"
./test/test_rand_io test/conf/run_cache_virt.txt test read_percent=0

echo "the basic test for global cached IO with small reads on virtual SSDs"
./test/test_rand_io test/conf/run_cache_virt.txt test pages=$((1024 * 1024)) entry_size=$((128))
...
./test/test_rand_io test/conf/run_cache_virt.txt test entry_size=$((4096 * 32)) RAID_block_size=64K
...
./test/test_rand_io test/conf/run_cache_virt.txt test pages=$((1024 * 1024)) entry_size=$((128)) read_percent=0

echo "the basic test for global cached IO with large writes on virtual SSDs."
./test/test_rand_io test/conf/run_cache_virt.txt test read_percent=0 entry_size=$((4096 * 32)) RAID_block_size=64K

echo "the basic test for global cached IO under the TPCC workload."
./test/test_rand_io test/conf/run_cache_tpcc.txt test

echo "the basic test for global cached IO under the Neo4j workload."
./test/test_rand_io test/conf/run_cache_tpcc.txt test workload=test/workload/DijkstraSearch-reconstructed.data

echo "the basic test for parted global cached read on virtual SSDs."
./test/test_rand_io test/conf/run_parted_cache_virt.txt test

echo "the basic test for parted global cached IO under the TPCC workload on virtual SSDs."
./test/test_rand_io test/conf/run_parted_cache_tpcc.txt test
