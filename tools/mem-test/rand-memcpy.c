...
long rand_memcpy(char *src, char *dst, long start_i)
{
	...
	printf("copy %ld bytes, start at %f seconds, takes %f seconds\n",
...
int main(int argc, char *argv[])
{
	...
	node = atoi(argv[1]);
	offset = (off_t *) malloc(sizeof(*offset) * nentries);
...
//	permute_offset(offset, nentries);
	...
	bind2node_id(0);
	array0 = (char *) numa_alloc_local(ARRAY_SIZE);
	...
	bind2node_id(node);
	dst_arr = (char *) numa_alloc_local(ARRAY_SIZE);
	/* we need to avoid the cost of page fault. */
	for (i = 0; i < ARRAY_SIZE; i += PAGE_SIZE)
		dst_arr[i] = 0;
