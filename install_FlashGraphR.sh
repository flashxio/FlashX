...
if [ -n "$path" ]; then
	path=`dirname $path`
	echo $path
	RCPP_INCLUDE=$path FG_DIR=`pwd` FG_LIB=`pwd`/build R CMD INSTALL FlashGraphR_0.1-0.tar.gz
else
	FG_DIR=`pwd` FG_LIB=`pwd`/build R CMD INSTALL FlashGraphR_0.1-0.tar.gz
fi
