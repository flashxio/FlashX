...
public:
	...
	virtual void Apply(const Anasazi::MultiVec<double>& x,
...
void FM_Operator::Apply(const Anasazi::MultiVec<double>& x,
		Anasazi::MultiVec<double>& y) const
{
	const FM_MultiVector<double> &fm_x
		= dynamic_cast<const FM_MultiVector<double> &>(x);
	FM_MultiVector<double> &fm_y = dynamic_cast<FM_MultiVector<double> &>(y);
	mat->multiply_matrix<double>(fm_x.get_data(), fm_y.get_data());
}
...
main (int argc, char *argv[])
{
	...
	typedef Anasazi::MultiVecTraits<double, Anasazi::MultiVec<double> > MVT;
